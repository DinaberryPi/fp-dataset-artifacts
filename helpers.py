...
# This function preprocesses an NLI dataset using ONLY the hypothesis (for artifact model)
def prepare_dataset_nli_hypothesis_only(examples, tokenizer, max_seq_length=None):
    max_seq_length = tokenizer.model_max_length if max_seq_length is None else max_seq_length

    # Only tokenize hypothesis, ignore premise - this will learn artifacts!
    tokenized_examples = tokenizer(
        examples['hypothesis'],
        truncation=True,
        max_length=max_seq_length,
